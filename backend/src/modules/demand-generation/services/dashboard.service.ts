...
import { marketingFiscalRange } from '../lib/marketing-fiscal-period';
...
export class DashboardService {
  ...
  async getMarketingDashboard(
    ...
    const [
      ...
      activeLeadDateRange
        ? this.ouvMarketingMetrics.averageLeadToOuvDays(
  ...
  async getMarketingDashboardDetails(
    ...
    if (query.kind === 'period_leads') {
      ...
      return this.listLeadDetails(
        this.captureRangeWhere(activeRange.start, activeRange.end),
        page,
        limit,
        query.kind,
      );
    ...
    if (query.kind === 'quarter_leads') {
      ...
      return this.listLeadDetails(
        this.captureRangeWhere(ranges.quarterStart, ranges.quarterEnd),
        page,
        limit,
        query.kind,
      );
  ...
  private captureRangeWhere(start: Date, end: Date): WhereOptions<Lead> {
    return {
      fechaCaptura: { [Op.gte]: start, [Op.lt]: end },
    };
  }

  private interactionRangeWhere(start: Date, end: Date): WhereOptions {
    return {
  ...
  private buildFilteredLeadWhere(
    ...
    if (ranges.hasPeriodRange && ranges.periodStart && ranges.periodEnd) {
      return this.captureRangeWhere(ranges.periodStart, ranges.periodEnd);
    }
    if (
      ranges.hasAccumulatedRange &&
      ranges.quarterStart &&
      ...
      return this.captureRangeWhere(ranges.quarterStart, ranges.quarterEnd);
    }
    return {};
  }
  ...
  private async buildWeeklyMetrics(
    ...
    const metricLeadWhere: WhereOptions<Lead> | null = metricRange
      ? this.captureRangeWhere(metricRange.start, metricRange.end)
      : null;
    ...
    const [
      ...
      metricRange
        ? this.interactionModel.count({
            where: this.interactionRangeWhere(metricRange.start, metricRange.end),
          })
        : Promise.resolve(0),
      metricLeadWhere
  ...
  private resolveRanges(
    ...
    const fiscalQuarter = quarter == null ? null : Number(quarter);
    ...
    const selectedYear =
      Number.isInteger(fiscalYear) && fiscalYear >= 2000 ? fiscalYear : null;
    ...
    if (selectedYear != null) {
      ...
      const range = marketingFiscalRange(
        selectedYear,
        Number.isInteger(fiscalQuarter) ? fiscalQuarter : null,
      );
      quarterStart = range.start;
      quarterEnd = range.end;
      selectedQuarter = Number.isInteger(fiscalQuarter) ? fiscalQuarter : null;
    }
  ...
  private async listInteractionDetails(
    ...
    const { rows, count } = await this.interactionModel.findAndCountAll({
      where: this.interactionRangeWhere(from, to),
      include: [
        {
          model: Lead,
          as: 'lead',
  ...
  private async countInteractionsByChannel(start: Date, end: Date) {
    const rows = await this.interactionModel.findAll({
      ...
      where: this.interactionRangeWhere(start, end),
