...
export function Sidebar() {
  const commercial = NAV_ITEMS.filter((i) => i.group === 'commercial');
  const platform = NAV_ITEMS.filter((i) => i.group === 'platform');
  ...
  return (
    <aside className="flex h-screen w-64 flex-col border-r border-border bg-surface">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-2">
        ...
        {commercial.map((item) => <Item key={item.key} item={item} />)}

        <SectionLabel className="mt-4">Plataforma</SectionLabel>
        {platform.map((item) => <Item key={item.key} item={item} />)}
      </nav>
...
function SectionLabel({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <p className={`px-3 pb-1 pt-2 text-[11px] font-bold uppercase tracking-wide text-muted ${className}`}>
      {children}
    </p>
  );
}
